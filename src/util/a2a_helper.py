...
async def wait_agent_ready(url, timeout=10):
    """Wait until A2A server is ready by checking agent card."""
    retry_cnt = 0
...
async def send_message(
    ...
    card = await get_agent_card(url)
    # Set very long HTTP timeout (1 hour) - rely on agent timeout instead
    httpx_client = httpx.AsyncClient(timeout=7200.0)
